...
def evaluate_lenet5(learning_rate=0.02, n_epochs=100, emb_size=300, batch_size=50, filter_size=[3], sent_len=40, claim_len=20, cand_size=10,hidden_size=[300,300], max_pred_pick=5):
    ...
    '''
    sent context, we need add a mask
    '''
    batch_dot = T.batched_dot(batch_sent_emb, batch_sent_emb.dimshuffle(0,2,1)) #(batch, cand_size, cand_size)
    batch_dot_softmax = T.nnet.softmax(batch_dot.reshape((batch_size*cand_size, cand_size))).reshape((batch_size, cand_size, cand_size))
    # batch_context = T.batched_dot(batch_dot_softmax*sents_labels.dimshuffle(0,'x',1), batch_sent_emb) #(batch, cand_size, hidden_size)
    batch_context = T.batched_dot(batch_dot_softmax, batch_sent_emb) #(batch, cand_size, hidden_size)

    concate_claim_sent_context = T.concatenate([batch_claim_emb,batch_sent_emb, batch_context], axis=2)
    concate_2_matrix = concate_claim_sent_context.reshape((batch_size*cand_size, hidden_size[0]*3))
