...
from preprocess import compute_f1_two_list_names,compute_f1_recall_two_list_names,count_sent_page
...
def evaluate_lenet5(learning_rate=0.02, n_epochs=100, emb_size=300, batch_size=50, filter_size=[3], sent_len=40, claim_len=40, cand_size=10,hidden_size=[300,300], max_pred_pick=5):
    ...
    dev_3th_sents, dev_3th_sent_masks, dev_3th_sent_labels, dev_3th_claims, dev_3th_claim_mask, dev_3th_labels, word2id = load_fever_test_NoEnoughInfo(sent_len, claim_len, cand_size, word2id)
    ...
    dev_3th_sent_masks=np.asarray(dev_3th_sent_masks, dtype=theano.config.floatX)
    ...
    joint_dev_sent_labels = np.concatenate((dev_sent_labels,dev_3th_sent_labels))
    ...
    dev_3th_claim_mask=np.asarray(dev_3th_claim_mask, dtype=theano.config.floatX)
    ...
    dev_3th_batch_start=list(np.arange(n_dev_3th_batches)*batch_size)+[dev_3th_size-batch_size]
        ...
        for joint_batch_id in joint_train_batch_start: #for each batch
            ...
            if iter%100==0:
                ...
                if f1 > max_test_f1 or strict_score > max_test_acc:
                    ...
                    full_evi = 0
                    predictions = []
                    fine_grained_sent_predictions = {1:[],2:[],3:[],4:[],5:[]}
                    fine_grained_page_predictions = {1:[],2:[],3:[],4:[]}
                    for dev_batch_id in dev_batch_start: # for each test batch
                        batch_prob, error_i, pred_i=dev_model(
                                ...
                                dev_sent_masks[dev_batch_id:dev_batch_id+batch_size],
                                dev_sent_labels[dev_batch_id:dev_batch_id+batch_size],
                                dev_claims[dev_batch_id:dev_batch_id+batch_size],
                                dev_claim_mask[dev_batch_id:dev_batch_id+batch_size],
                                dev_labels[dev_batch_id:dev_batch_id+batch_size]
                                )
                        error_sum+=error_i
                        batch_sent_labels = dev_sent_labels[dev_batch_id:dev_batch_id+batch_size]
                        batch_sent_names = dev_sent_names[dev_batch_id:dev_batch_id+batch_size]
                        batch_ground_names = dev_ground_names[dev_batch_id:dev_batch_id+batch_size]
                        batch_ground_labels = dev_labels[dev_batch_id:dev_batch_id+batch_size]
                        for i in range(batch_size):
                            instance_i = {}
                            instance_i['label'] = pred_id2label.get(batch_ground_labels[i])
                            instance_i['predicted_label'] = pred_id2label.get(pred_i[i])
                            ...
                            evi_sent_size, evi_page_size = count_sent_page(gold_sent_names)
                            fine_grained_sent_predictions.get(evi_sent_size).append(instance_i)
                            fine_grained_page_predictions.get(evi_page_size).append(instance_i)
                    strict_score, label_accuracy, precision, recall, f1 = fever_score(predictions)
                    print 'strict_score, label_accuracy, precision, recall, f1: ', strict_score, label_accuracy, precision, recall, f1
                    print '......sent...\n'
                    for i in range(1,6):
                        ...
                        if len(predictions_i)>0:
                            strict_score, label_accuracy, precision, recall, f1 = fever_score(predictions_i)
                    ...
                    for dev_batch_id in dev_3th_batch_start: # for each test batch
                        _, error_i, pred_i=dev_model(
                                dev_3th_sents[dev_batch_id:dev_batch_id+batch_size],
                                dev_3th_sent_masks[dev_batch_id:dev_batch_id+batch_size],
                                dev_3th_sent_labels[dev_batch_id:dev_batch_id+batch_size],
                                dev_3th_claims[dev_batch_id:dev_batch_id+batch_size],
                                dev_3th_claim_mask[dev_batch_id:dev_batch_id+batch_size],
                                dev_3th_labels[dev_batch_id:dev_batch_id+batch_size]
                                )
                        for i in range(batch_size):
                            instance_i = {}
                            instance_i['label'] = pred_id2label.get(2)
